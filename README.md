...
| `NHPLUG_WS_MAX_KEYS` | 세션당 실시간 등록 수. 기본·상한은 [서버 한도](docs/realtime_channels.md#서버-한도) (**낮추는 것만** 가능) |
| `NHPLUG_WS_MAX_SESSIONS` | 동시 WebSocket 세션. 기본·상한은 [서버 한도](docs/realtime_channels.md#서버-한도) (**낮추는 것만** 가능) |
...
> 🔴 **채널코드는 대소문자를 구분합니다.** 해외주식 지연채널 `rh`·`rc` 와 국내파생 지수옵션 **미니** `rH`·`rC`·`rE` 는 대소문자만 다르고 **포트가 다릅니다**(7080 / 7070). 코드에서 `tr_cd` 를 소문자로 바꾸지 마세요.
...
`subscribe()` 가 알아서 처리하므로 **한도를 신경 쓰지 않아도 됩니다.**

- 종목이 세션당 한도를 넘으면 **나눠서 여러 세션**으로 구독
- 동시 세션 수를 넘지 않음(초과분은 앞 세션이 끝나면 이어서)
- 구독 전송 간격 제어
- 종료 시 `tr_type=2` 로 **등록 반납**

> 한도 값(동시 세션 · 세션당 등록 · 전송 속도)은 **[docs/realtime_channels.md](docs/realtime_channels.md#서버-한도)** 에 정리돼 있습니다.
> 서버가 강제하는 값이라 **환경변수로 올릴 수 없고** 낮추는 것만 가능합니다.
