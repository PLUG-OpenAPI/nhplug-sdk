...
> 정본은 [openapi.json](https://www.nhplug.com/openapi-docs/krbond/openapi.json). ⚠️ 실주문은 실거래(api...) 에서만 체결 — 개발은 테스트/모의 우선.
...
테스트 Endpoint: `wss://devapi.nhplug.com:7070` · 구독·인증·푸시 규약은 [openapi.json](https://www.nhplug.com/openapi-docs/krbond/openapi.json) 의 `x-realtime-channels.protocol` 참조.

| 채널 | tr_cd | tr_key | 응답 필드 수 |
|---|---|---|---|
| 장내채권 실시간 호가소액 | `c1` | `expcode`(종목코드) | 35 |
| 장내채권 실시간 호가소액 | `c3` | `expcode`(표준채권코드) | 34 |
| 장내채권 실시간 체결가소액 | `c2` | `expcode`(종목코드) | 15 |
| 장내채권 실시간 체결가소액 | `c4` | `expcode`(표준채권코드) | 14 |
