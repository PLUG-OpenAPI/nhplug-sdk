...
> 📌 **이 표가 한도를 적는 유일한 문서입니다.** 다른 문서는 여기로 링크만 합니다
> (코드 정본은 `nhplug/realtime.py` 의 `MAX_*` 상수이고, 일관성 검사가 둘을 대조합니다).

| 항목 | 한도 | 초과 시 |
|---|---|---|
| 앱키당 동시 세션 | **2** | `WSS10015` |
| 세션당 실시간 등록 | **30** | close code 1000 `"Bye"` — 오류 메시지 없이 끊김 |
| 구독 전송 | **초당 10건** | `WSS10010` |

⚠️ **아래 두 값은 서로 다릅니다.** 세션당 등록은 **30건**, 구독 전송 속도는 **초당 10건** —
둘 다 "10"이던 시절의 서술이 남아 있으면 의심하세요. (명세 260911 에서 등록 한도가 10 → 30 으로 올랐습니다.)
...
### 🔴 정규장 기준값이 추가됐습니다 (명세 260911 · KRX 시간연장)

거래시간이 연장돼 **정규장외 체결이 같은 채널로 섞여 내려옵니다.** 그래서 "정규장 기준" 값을 따로 줍니다.

| 채널 | 추가 필드 | 뜻 |
|---|---|---|
| 체결가 `oc`·`nc`·`mc` (24 → **29**) | `marketgb` | 시장구분 — 이 체결이 어느 시장인지 |
| | `main_close` | 정규장 종가 |
| | `market_sign`·`market_change`·`market_chrate` | 정규장 기준 대비부호·대비·등락률 |
| 예상체결 `oa`·`na` (16 → **17**)<br>`ma` (19 → **20**) | `vi_exch_code` | VI 발동 거래소 구분 |

> SDK 는 푸시 본문을 **그대로** 콜백에 넘기므로 코드 수정 없이 새 필드가 들어옵니다.
> `body.get("main_close")` 처럼 바로 읽으면 됩니다.
>
> ⚠️ **REST 쪽은 다릅니다** — 시세·잔고 조회에는 `view_main_yn`·`aly_qut_cd` **필수 파라미터**가
> 새로 생겼습니다. 빠뜨리면 호출이 실패합니다. [README](../README.md) 참조.
